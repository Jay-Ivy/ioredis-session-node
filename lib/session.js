...
let generate = function (req, res) {
    ...
    session[SESSION.CSRF] = generateCSRF();
    req.session = session;
    writeHead(req, res);
    return session;
};
...
let writeHead = function (req, res) {
    if (req.session && req.session.id) {
        ...
        res.cookie(SESSION.CSRF, req.session[SESSION.CSRF], {
            httpOnly: config.httpOnly,
            path: config.path,
        });
    }
...
module.exports = function session(options) {
    ...
    if (!config.redisNodes || config.redisNodes.length == 0) {
        console.debug('not redis nodes info');
    ...
    return (req, res, next) => {
        ...
        if (!id) {
            ...
            redisCluster.hget(id, 'session').then((reply) => {
                ...
                let referer = req.headers.referer || getDomain(req),
                    host = req.headers.host,
                    csrf = session[SESSION.CSRF];
                // 验证csrf标识
                if (new RegExp(host).test(referer) && req.cookies[SESSION.CSRF] == csrf) {
                    req.session = session;
                    if (config.refreshSession) {
                        save(req, res, next);
                    } else {
