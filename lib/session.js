    ...
    return (req, res, next) => {
        ...
        if (!id) {
            ...
            redisCluster.hget(id, 'session').then((reply) => {
                ...
                if (config.csrf.able) {
                    // 验证csrf标识
                    let referer = req.headers.referer || getDomain(req), csrf = session[config.csrf.key];
                    if (new RegExp(req.headers.host).test(referer) && req.cookies[config.csrf.key] == csrf) {
                        req.session = session;
                        if (config.keepAlive) {
                            save(req, res, next);
                        } else {
